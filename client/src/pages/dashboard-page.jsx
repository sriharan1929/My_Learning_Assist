...
import { ArrowUpRight, CheckCircle2, Clock3, Layers3, Target, Flame } from "lucide-react";
...
export function DashboardPage() {
  ...
  return <><PageHeader eyebrow="Your learning command center" title="Good to see you." description="A clear view of what matters today and how your learning is moving." action={<Button as="a"><Link to="/focus">Start a focus session <ArrowUpRight size={16} /></Link></Button>} />
    <SectionGrid columns="five"><Stat label="Overall progress" value={`${data.stats.progress}%`} icon={Target} /><Stat label="Learning items" value={data.stats.items} icon={Layers3} /><Stat label="Completed" value={data.stats.completed} icon={CheckCircle2} /><Stat label="Focus time" value={`${data.focusMinutes}m`} icon={Clock3} /><Stat label="Habit streak" value={`${data.stats.currentStreak || 0}d`} icon={Flame} accent /></SectionGrid>
